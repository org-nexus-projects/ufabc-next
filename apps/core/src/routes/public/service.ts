...
import type { GraduationList } from '@/schemas/public.js';
...
export async function listPublicGraduations() {
...
export async function buildStudentStats(season: string) {
  if (!season) {
    throw httpErrors.badRequest('Missing season');
  }
...
export async function buildComponentsStats(
  action: 'overview' | 'component' | 'courses' | undefined,
  query: ComponentsStatsQuery
) {
  const { limit, page, season, courseId, ratio, turno } = query;
  ...
  const [result] = await ComponentModel.aggregate<Record<string, unknown>>(
    pipeline
  );
  return result;
}
