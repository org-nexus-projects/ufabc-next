...
type CourseIdentifier = number | string;

export async function listCurrentSeasonCourses() {
  const season = currentQuad();
  const coursesAggregate = await StudentModel.aggregate<{
    _id: string;
    ids: Array<CourseIdentifier | null>;
  }>([
    { $match: { season } },
    { $unwind: '$cursos' },
  ...
  const courses = coursesAggregate.map((course) => {
    const validIds = course.ids.filter(
      (id): id is CourseIdentifier => id != null && id !== ''
    );
...
function findMode(arr: CourseIdentifier[]) {
  const frequencyMap = arr.reduce<Record<string, number>>((acc, val) => {
    acc[val] = (acc[val] || 0) + 1;
    return acc;
  }, {});
