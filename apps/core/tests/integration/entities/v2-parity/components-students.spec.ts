...
import { omitDeep, sortListsDeep } from './fixtures.js';
import {
  type CasoComponentsStudents,
  casosComponentsStudents,
  seedComponentsStudents,
  snapshotComponentsStudents,
} from './fixtures-components-students.js';
...
describe('paridade V1/V2: components e students', () => {
  ...
  async function executar(
    ...
    await seedComponentsStudents();
    await app.redis.flushdb();

    const res = await app.inject({
      headers: {
        ...(caso.credencial ? { 'session-id': 'sessao-teste' } : {}),
        ...caso.headers,
      },
      method: caso.method,
    ...
    return {
      banco: omitDeep(await snapshotComponentsStudents(), ignorar),
  ...
  it.each(casosComponentsStudents)('$nome', async (caso) => {
