import type {
  SearchSubject,
  SearchTeacher,
  SubjectInfo,
  TeacherReview,
  TeacherSummary,
} from './types';

import { api } from './api';

export const Reviews = {
  searchTeachers: async (q: string) => {
    const { data } = await api.get('/v2/entities/teachers/search', {
      params: { q },
    });
    return data as SearchTeacher;
  },
  searchSubjects: async (q: string) => {
    const { data } = await api.get('/v2/entities/subjects/search', {
      params: { q },
    });
    return data as SearchSubject;
  },
  getTeacher: async (id: string) => {
    const { data } = await api.get(`/v2/entities/teachers/reviews/${id}`);
    return data as TeacherReview;
  },
  getSubject: async (id: string) => {
    const { data } = await api.get(`/v2/entities/subjects/reviews/${id}`);
    return data as SubjectInfo;
  },
  getTeacherSummary: async (id: string) => {
    const { data } = await api.get(`/v2/teachers/${id}/summary`);
    return data as TeacherSummary;
  },
};